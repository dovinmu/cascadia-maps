...
(function() {
  ...
  makeProjection = function() {
    ...
    projection = d3.geo.mercator().scale(width * scaling).center([-124.75, 49.5]).translate([0, 0]);
  ...
  positionText = function() {
    ...
    mapTitle.attr('x', coords[0]).attr('y', coords[1]).style('font-size', tinymode ? '22px' : '50px');
    if (tinymode) {
      mapTitle.text('Washington evergreens');
    }
    coords = projection([titleLon + 0.08, titleLat - 0.13]);
    mapDescription.attr('x', coords[0]).attr('y', coords[1]).attr('opacity', tinymode ? 0 : 1);
    if (portrait) {
      treeMenuLat = 45.33;
      treeMenuLon = -124.7;
    } else {
    ...
    diff = 12;
    treeMenuText.style("font-size", '18px').attr('x', coordsText[0]).attr('y', coordsText[1]);
    treeMenuOverflowText.style("font-size", '18px').attr('x', coordsOverflow[0]).attr('y', coordsOverflow[1]);
    for (i = k = 0, len = disclaimer.length; k < len; i = ++k) {
      line = disclaimer[i];
      line.style("font-size", tinymode ? '10px' : '11px').attr("x", coordsParagraph[0]).attr("y", coordsParagraph[1] + (i + 2) * diff);
    }
    for (i = l = 0, len1 = quote.length; l < len1; i = ++l) {
      line = quote[i];
      line.style("font-size", tinymode ? '10px' : '11px').attr("x", coordsParagraph[0]).attr("y", coordsParagraph[1] + i * diff);
    }
    ...
    if (tinymode) {
      d3.select('#info').style('padding', '4px').style('font-size', '8px');
  ...
  drawTreeMenu = function(treeId, selected) {
    ...
    svg.style('width', width + 'px').style('height', bottom + 'px');
    return console.log("resized to", width, bottom);
  };
