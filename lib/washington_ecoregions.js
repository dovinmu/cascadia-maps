...
(function() {
  var changeText, getClassNameEco, getClassNameTree, getColor, getSubClassNameEco, height, initMap, loadJson, onClickEco, onClickTree, overflowText, overflow_limit, path, processTree, projection, region_trees, removeImages, selected, selectedText, selectedTextDetail, selectedTree, shadeColor2, showImage, sizes, splitText, svg, tree_regions, width;
  ...
  onClickTree = function(d, i) {
    var k, l, len, len1, region, region_list, results;
    if (selectedTree) {
    ...
    for (l = 0, len1 = region_list.length; l < len1; l++) {
      ...
      results.push(d3.selectAll('.' + getSubClassNameEco(region)).style('opacity', '1'));
    }
    return results;
  };
  ...
  overflowText = svg.append('text').attr('x', width - width / 4.5).attr('y', 90.).attr('class', 'selected title').text('');
  ...
  changeText = function(text, textDetail) {
    ...
    selectedText.transition().duration(50).style('opacity', 0).transition().duration(250).style('opacity', 1).text(text);
    overflowText.transition().duration(50).style('opacity', 0).transition().duration(250).style('opacity', 1).text(overflow);
    if (!textDetail) {
      textDetail = '';
    }
    return selectedTextDetail.transition().duration(50).style('opacity', 0).transition().duration(250).style('opacity', 1).text(textDetail);
  };
