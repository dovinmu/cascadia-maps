...
(function() {
  var currentTreeId, desertLabel, disclaimer, drawTreeMenu, getClassNameEco, getClassNameTree, getColor, getSubClassNameEco, height, hideLabels, hideTreeMenuText, initMap, lightenColor, loadJson, makeProjection, mapDescription, mapTitle, marineLabel, mountainLabel, onClickEco, onClickTree, overflow_limit, path, portrait, positionText, processTree, projection, quote, region_trees, removeImages, resize, selected, selectedTree, setTitleAndDescription, setTreeMenuText, shadeColor2, showDisclaimer, showImage, showLabels, showQuote, sizes, splitText, svg, treeMenuLat, treeMenuLon, treeMenuOverflowText, treeMenuText, tree_regions, trees, width;
  ...
  makeProjection = function() {
    ...
    projection = d3.geo.mercator().scale(width * scaling).center([-120.5, 47.75]).translate([xTrans, yTrans]);
  ...
  positionText = function() {
    var coords, coordsOverflow, coordsParagraph, coordsText, diff, i, k, l, len, len1, line, titleLat, titleLon;
    if (portrait) {
      titleLat = 49.2;
      titleLon = -124.2;
    } else {
  ...
  drawTreeMenu = function(treeId, selected) {
    var i, k, len, results, tree, tree_list;
  ...
  showImage = function(name, i) {
    var fname, horizontalCount, id, startCoords, stride, x, y;
    ...
    if (portrait) {
      horizontalCount = Math.floor(width / stride[0]);
    } else {
      horizontalCount = 3;
    }
    console.log("we can fit", horizontalCount, "trees");
    y = startCoords[1] + (Math.floor(i / horizontalCount)) * stride[1];
