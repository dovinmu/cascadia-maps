...
(function() {
  var desertLabel, disclaimer, getClassNameEco, getClassNameTree, getColor, getSubClassNameEco, height, hideLabels, hideTreeMenuText, initMap, lightenColor, loadJson, mapDescription, mapTitle, marineLabel, mountainLabel, onClickEco, onClickTree, overflow_limit, path, processTree, projection, quote, region_trees, removeImages, selected, selectedTree, setTitleAndDescription, setTreeMenuText, shadeColor2, showDisclaimer, showImage, showLabels, showQuote, sizes, splitText, svg, treeMenuOverflowText, treeMenuText, tree_regions, trees, width;
  ...
  mapTitle = svg.append('text').attr('x', width / 6).attr('y', 50.).attr('class', 'mapname').text('Washington state evergreens');

  mapDescription = svg.append('text').attr('x', width / 6 + 10).attr('y', 75.).attr('class', 'selected detail').text('Click on an ecoregion to see the list of evergreen trees native to it.');
  ...
  initMap = function(error, ecotopo) {
    ...
    for (i = l = 0, len1 = ref1.length; l < len1; i = ++l) {
      line = ref1[i];
      quote.push(svg.append("text").attr("x", width - width / 4.5).attr("y", 110. + i * 20).attr("class", "quote").style('opacity', 0).text(line));
    }
    ref2 = data.features;
